// Xử lý quiz Lịch sử & Địa lí - Giữa kì 1 năm học 2025-2026
function startLichSuDiaLiGK1_2025_2026_Quiz() {
  ...
  Promise.all([
    fetch('data/questions_lichsu_dialy_gk1_2025_2026.json').then(r => r.json()),
    fetch('data/true_false_lichsu_dialy_gk1_2025_2026.json').then(r => r.json()),
    fetch('data/short_answer_lichsu_dialy_gk1_2025_2026.json').then(r => r.json()),
    fetch('data/essay_questions_lichsu_dialy_gk1_2025_2026.json').then(r => r.json())
  ])
    .then(([mcQuestions, tfQuestions, saQuestions, essayQuestions]) => {
      ...
      showQuizModal_2025(allQuestions, 'Lịch sử và Địa lí - Thi Giữa kì 1 (2025-2026)');
    })
    .catch(error => {
...
function showQuizModal_2025(questions, title) {
  ...
  modal.innerHTML = `
    <div class="quiz-content">
      <div class="quiz-header">
        <h2>${title}</h2>
        <button class="close-btn" onclick="closeQuizModal_2025()">&times;</button>
      </div>
      <div class="quiz-body">
        <div class="question-info">
          <span class="question-number">Câu <span id="current-q-2025">1</span>/${questions.length}</span>
          <span class="question-type" id="question-type-2025"></span>
          <span class="question-points" id="question-points-2025"></span>
        </div>
        <div class="question-text" id="question-text-2025"></div>
        <div class="choices" id="choices-2025"></div>
      </div>
      <div class="quiz-footer">
        <button id="prev-btn-2025" onclick="prevQuestion_2025()" disabled>Câu trước</button>
        <button id="next-btn-2025" onclick="nextQuestion_2025()">Câu sau</button>
        <button id="submit-btn-2025" onclick="submitQuiz_2025()" style="display:none;">Nộp bài</button>
      </div>
      <div class="quiz-result" id="result-2025" style="display:none;"></div>
    </div>
  `;
  ...
  function showQuestion() {
    ...
    document.getElementById('question-text-2025').innerHTML = q.question;
    document.getElementById('current-q-2025').textContent = currentQuestion + 1;
    ...
    document.getElementById('question-type-2025').textContent = `[${typeNames[q.type] || 'Trắc nghiệm'}]`;
    document.getElementById('question-points-2025').textContent = `(${q.points || 0.25} điểm)`;

    const choicesDiv = document.getElementById('choices-2025');
    ...
    if (q.type === 'multiple-choice') {
      ...
      q.choices.forEach((choice, index) => {
        ...
        choiceDiv.innerHTML = `
          <input type="radio" name="answer-2025" id="choice-2025-${index}" value="${index}" 
            ${userAnswers[currentQuestion] === index ? 'checked' : ''}>
          <label for="choice-2025-${index}">${choice}</label>
        `;
        choiceDiv.onclick = () => {
          document.getElementById(`choice-2025-${index}`).checked = true;
      ...
      q.statements.forEach((stmt, index) => {
        ...
        stmtDiv.innerHTML = `
          ...
          <div class="statement-choices">
            <label>
              <input type="radio" name="stmt-2025-${index}" value="true" 
                ${userAnswers[currentQuestion]?.[index] === true ? 'checked' : ''}>
              Đúng
            </label>
            <label>
              <input type="radio" name="stmt-2025-${index}" value="false" 
    ...
    // Update buttons
    document.getElementById('prev-btn-2025').disabled = currentQuestion === 0;
    document.getElementById('next-btn-2025').style.display = currentQuestion < questions.length - 1 ? 'inline-block' : 'none';
    document.getElementById('submit-btn-2025').style.display = currentQuestion === questions.length - 1 ? 'inline-block' : 'none';
  }

  window.nextQuestion_2025 = function() {
  ...
  window.prevQuestion_2025 = function() {
  ...
  window.submitQuiz_2025 = function() {
    // Tính điểm
    score = 0;
    let correctCount = 0;
    let totalPoints = 0;
    ...
    const resultDiv = document.getElementById('result-2025');
    ...
    resultDiv.innerHTML = `
      ...
      <div class="result-buttons">
        <button onclick="closeQuizModal_2025()" class="btn-primary">Đóng</button>
      </div>
    `;
  ...
  window.closeQuizModal_2025 = function() {
    modal.remove();
    delete window.nextQuestion_2025;
    delete window.prevQuestion_2025;
    delete window.submitQuiz_2025;
    delete window.closeQuizModal_2025;
  };
}
