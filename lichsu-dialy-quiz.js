// Xử lý quiz Lịch sử & Địa lí - Tổng hợp tất cả loại câu hỏi
function startLichSuDiaLiCK1Quiz() {
  ...
  Promise.all([
    fetch('data/questions_lichsu_dialy_ck1_2024_2025.json').then(r => r.json()),
    fetch('data/true_false_lichsu_dialy_ck1_2024_2025.json').then(r => r.json()),
    fetch('data/short_answer_lichsu_dialy_ck1_2024_2025.json').then(r => r.json()),
    fetch('data/essay_questions_lichsu_dialy_ck1_2024_2025.json').then(r => r.json())
  ])
    .then(([mcQuestions, tfQuestions, saQuestions, essayQuestions]) => {
      ...
      showQuizModal(allQuestions, 'Lịch sử và Địa lí - Thi Cuối kì 1 (2024-2025)');
    })
    .catch(error => {
...
function showQuizModal(questions, title) {
  ...
  function showQuestion() {
    ...
    document.getElementById('question-points').textContent = `(${q.points || 1} điểm)`;
  ...
  window.submitQuiz = function() {
      ...
      if (q.type === 'multiple-choice') {
        ...
        answerHtml = `
          <div class="answer-item ${isCorrect ? 'correct' : 'incorrect'}">
            <div class="answer-header">
              <strong>Câu ${i + 1}:</strong> [Trắc nghiệm - ${q.points || 1} điểm]
            </div>
        ...
        answerHtml = `
          <div class="answer-item ${allCorrect ? 'correct' : 'incorrect'}">
            <div class="answer-header">
              <strong>Câu ${i + 1}:</strong> [Đúng/Sai - ${q.points || 1} điểm]
            </div>
            <div class="answer-question">${q.question}</div>
        ...
        answerHtml = `
          <div class="answer-item needs-review">
            <div class="answer-header">
              <strong>Câu ${i + 1}:</strong> [Trả lời ngắn - ${q.points || 0.5} điểm]
            </div>
            <div class="answer-question">${q.question}</div>
            <div class="answer-detail">
        ...
        answerHtml = `
          <div class="answer-item needs-review">
            <div class="answer-header">
              <strong>Câu ${i + 1}:</strong> [Tự luận - ${q.points || 1} điểm]
            </div>
            <div class="answer-question">${q.question}</div>
            ${dataTableHtml}
            <div class="answer-detail">
    ...
    resultDiv.innerHTML = `
      ...
      <div class="score-summary">
        <p class="score">Điểm trắc nghiệm: ${score.toFixed(1)}/${totalPoints} điểm</p>
